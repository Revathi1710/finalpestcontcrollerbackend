...
origin: [
    ...
    'http://localhost:3000','http://localhost:3001','https://builder.myorderz.com',
...
const mongourl = process.env.MONGO_URL || 
  "mongodb+srv://pestcontroller:pestcontroller123@cluster0.m0lxg84.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
...
app.post("/VSearchBuyer", async (req, res) => {
  ...
  const apiKey = "0c4835f9b7b34a1b895d8a15e8e9691c"; // replace with your actual API key
  ...
  try {
    ...
    // Step 1: Check vendors by pincode
    if (businessType === "Residential") {
      ...
      // Step 2: If no vendors found, use geolocation to find nearby vendors within 3km
      if (vendors.length === 0) {
      ...
      // For non-residential, return all approved and active vendors
      vendors = await Vendor.find({
        approved: true,
        expiryDate: { $gt: today },
      });
  ...
  //get data vendor
  app.post("/getvendorDetails", async (req, res) => {
    const { vendorId } = req.body;
  
    try {
      // Validate input
      if (!vendorId) {
        return res.status(400).json({ status: "error", message: "vendorId is required" });
      }
  
      // Find vendor by ID
      const vendor = await Vendor.findById(vendorId); // .findById is cleaner
  ...
  // API route to handle vendor details update
  app.post(
    "/updateVendorAllDetails/:vendorId",
    upload.fields([
    ...
    async (req, res) => {
      try {
        ...
        if (req.files["logoImage"]) {
          updateData.logo = req.files["logoImage"][0].filename;
        }
  
        // Handle property images upload
        if (req.files["propertyImages"]) {
          updateData.image = req.files["propertyImages"].map(file => file.filename);
        }
        ...
        res.json({ status: "ok", message: "Vendor details updated successfully" });
      } catch (error) {
        console.error("Error updating vendor details:", error); // Log detailed error for debugging
        res.status(500).json({ status: "error", message: error.message });
      }
    }
  );
 ...
 //GET SLUG details
 app.post("/getvendorSlug", async (req, res) => {
  const { businessSlug } = req.body;

  try {
    if (!businessSlug) {
      return res.status(400).json({ status: "error", message: "businessSlug is required" });
    }

    const vendor = await Vendor.findOne({ businessSlug });
    ...
    console.error("Error in /getvendorSlug:", error);
    res.status(500).json({ status: "error", message: "Server error" });
  }
});
app.put('/onboardVendor', async (req, res) => {
  try {
    const { vendorId, businessType, expiryDays, startDate, expiryDate } = req.body;

    // Validate incoming data
    if (!vendorId || !businessType || !expiryDays || !startDate || !expiryDate) {
      return res.status(400).json({ status: 'error', message: 'Missing required fields' });
    }
    ...
    if (isNaN(parsedStartDate.getTime()) || isNaN(parsedExpiryDate.getTime())) {
      return res.status(400).json({ status: 'error', message: 'Invalid startDate or expiryDate' });
    }
    ...
    vendor.expiryDays = expiryDays;
    vendor.startDate = parsedStartDate;
    vendor.expiryDate = parsedExpiryDate;

    // Save updated vendor
    await vendor.save();

    res.json({ status: 'ok', message: 'Vendor onboarded successfully' });
  } catch (err) {
...
app.get('/OnboardedVendor', async (req, res) => {
  try {
      const today = new Date();
      ...
      if (vendors.length === 0) {
          return res.status(404).send({ status: 'error', message: 'No vendors found' });
      }
...
app.get('/ExpiredVendor', async (req, res) => {
  try {
      const today = new Date();
      today.setHours(0, 0, 0, 0); // Normalize time for comparison
      ...
      // Change from $lt (less than) to $gt (greater than) to find vendors with expiryDate > today
      const vendors = await Vendor.find({
          expiryDate: { $lt: today },  // Ensure expiryDate is greater than today
          approved: true
      });
...
app.post('/addVendor', upload.fields([
  ...
  try {
    const {
      businessName, address, pincode, sinceFrom, specialistIn,
      contactPerson, contactNumber, email, pesticideLicence, gstNumber,
      membership, branchDetails, technicalQualification, password,
      aboutUs, website, latitude, longitude
    } = req.body;
...
// Get vendor by ID
app.get('/getVendor/:id', async (req, res) => {
  try {
...
app.delete("/deleteVendor", async (req, res) => {
  ...
  try {
      ...
      res.send({ status: 'ok', message: 'Vendor deleted successfully' });
  } catch (error) {
